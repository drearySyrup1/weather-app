export function create(parent) {
  parent.innerHTML = `
    ...
    >
      ...
      />
      <!-- <path class="cls-4" d="M117.78,101.23" /> -->
      <g class="groupy">
        <circle
          stroke-width="0"
          class="cls-5"
        ...
        <!-- <line class="line" x1="173" y1="25" x2="192.5" y2="25" />
        <line class="line" x1="182.83" y1="15" x2="182.83" y2="34.5" /> -->
      </g>
    </svg>`;
}
const cords = [
...
export const control = new Promise((resolve, reject) => {
  window.addEventListener('DOMContentLoaded', () => {
    const sun = document.querySelector('.groupy');
    sun.style.cssText = `
        --x: -162px;
        --y: 63px;
        ...
        opacity: .4;
      `;
    const X = parseInt(getComputedStyle(sun).getPropertyValue('--x'));
    const Y = parseInt(getComputedStyle(sun).getPropertyValue('--y'));
    const setX = (val) => {
      const value = X + val;
      sun.style.setProperty('--x', `${value}px`);
    };
    const setY = (val) => {
      const value = Y - val;
      sun.style.setProperty('--y', `${value}px`);
    };
    // ANIMATION;
    function animator(ms) {
      let count = 1;
      function asd(cb) {
        setTimeout(cb, ms * count);
        count++;
      }
      return asd;
    }
    const runner = animator(20);
    ...
    const getPercent = (prc) => {
      return Math.round(((cords.length - 1) * prc) / 100);
    };

    const setState = (prc) => {
      const xy = cords[getPercent(prc)];
      setX(xy.x);
      setY(xy.y);
    };

    resolve({ setState, animate });
  });
});
